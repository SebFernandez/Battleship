...
int main (int argc, char* argv[])	{
	int M_N1 [10][10] = {{0}}, M_N2[10][10] = {{0}}, turn = 0, flag = 0, M = 0, i, j;
	char D;
...
int WATER (int m_n1 [10][10], int m_n2 [10][10], int A, int B, int C, int D, int E, int TURN)	{
	int i = 0, flag = 0;
		if ((A == C) || (B == D))	{
			...
			} else if ((A == C) && (D > B) && (D - B == E))	{		//CHECK UP
				if (TURN == 0)	{
					for (i = B; i < D; i++)	{
						if (m_n1 [i][A] == 0)	{
							flag++;
			...
			} else if ((C > A) && (B == D) && (C - A == E))	{		//CHECK RIGHT
				if (TURN == 0)	{
					for (i = A; i < C; i++)	{
						if (m_n1 [B][i] == 0)	{
							flag++;
...
void FILL_PVP (int m_n1 [10][10], int m_n2 [10][10])	{
	int i, j, BO, fill = 0, x1, x2, y1, y2, flag = 0;
...
void FILL_PVC (int m_n1 [10][10], int m_n2 [10][10])	{
	int rn, i, j, BO, fill = 0, x1, x2, y1, y2, flag = 0;
	time_t t;
	SHIP S[4];
...
void GAME_PVP (int m_n1 [10][10],int m_n2 [10][10])	{
	int TURN = 0, f, c, PLAYER = 0, flag = 0, i = 0, j = 0, P1 = 0, P2 = 0;
